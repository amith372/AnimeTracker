...
- **Background work:** monthly sync is a scheduled Supabase Edge Function (`supabase/functions/mal-monthly-sync`, `pg_cron` + `pg_net`) — no per-device background job. `src/repositories/SyncRepository.ts` is just the "Check for new seasons" button's thin client of that function.
- **Images:** `expo-image`
...
11. **Monthly sync moves server-side** *(done)* — `supabase/functions/mal-monthly-sync` runs entirely server-side, scheduled by `pg_cron`/`pg_net`, and also serves the client's synchronous "Check for new seasons" button (per-user JWT mode). See Feature specs §6 for the full design (dual-mode auth, the narrow domain-logic-duplication exception). Unaffected by Phase 12 below — it already only ever touched Postgres.
