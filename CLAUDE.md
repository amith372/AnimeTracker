...
- **Background work:** Phase 11+: monthly sync is a scheduled Supabase Edge Function (`supabase/functions/mal-monthly-sync`, `pg_cron` + `pg_net`) — no per-device background job anymore. `expo-background-task`/`expo-task-manager` were removed from the project entirely; `src/repositories/SyncRepository.ts` is now just the "Sync now" button's thin client of that function.
- **Images:** `expo-image`
...
- **Client-side, this is now just a thin "Sync now" button**: `src/repositories/SyncRepository.ts`'s `runMonthlySync()` checks `isMalLinked()`, calls `callMalMonthlySync()` (→ `mal-monthly-sync`, authenticated per-user mode), and nudges `requestPull()` if it reports new seasons found — so the button's own result and the visibly-updated Library list land together rather than the list lagging a beat behind Realtime. The old per-device `expo-background-task` registration (and the client-side walk it used to trigger, including its use of `getAnimeDetail`/`getAnimeDetailCached`) is gone entirely — `expo-background-task` and `expo-task-manager` were uninstalled.
...
11. **Monthly sync moves server-side** *(done — client live-verified; `pg_cron` schedule created but not yet exercised by an actual monthly firing)* — `supabase/functions/mal-monthly-sync` replaces the per-device `expo-background-task` job entirely; `pg_cron`/`pg_net` fire it once a month (`supabase/migrations/20260808000000_monthly_sync_cron.sql`) via a `CRON_SECRET`-gated request, and the same function also serves the client's synchronous "Sync now" button (per-user JWT mode). See Feature specs §6 for the full design (dual-mode auth, the narrow domain-logic-duplication exception, and why the client side shrank to a two-line repository).
