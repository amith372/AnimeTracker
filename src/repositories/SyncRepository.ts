...
// normal Phase 10 pull/Realtime path instead, same as any other remote change, with no per-device
// wake-up or network call needed at all. The old per-device expo-background-task registration
// (registerBackgroundSync) is gone along with the client-side walk it used to trigger.
import { isMalLinked } from '@/account/malLinkRepository';
import { callMalMonthlySync } from '@/api/edgeFunctions';
import { requestPull } from '@/sync/pull';
...
export async function runMonthlySync(): Promise<number> {
  ...
  // The Edge Function wrote straight to Postgres, not this device's local SQLite — this device's
  // own Realtime subscription will pick the change up on its own shortly, but nudging a pull right
  // away means the button's "N series have new seasons" result and the visibly-updated Library
  // list land together instead of the list lagging a beat behind the message.
  if (seriesWithNewSeasons > 0) requestPull();
